...
def generate_pef_turing_pattern(params):
            ...
            for col in range(num_seeds_per_dim):
                ...
                for x in range(resolution):
                    for y in range(resolution):
                        ...
                        # else: no need to set outside, it's already 0 (initialized with zeros)
    else:
    ...
    for i in range(iterations):
        ...
        # Step 3: Update E_inhibitor (inhibitor reaction part).
        dE_inhibitor = P * 0.17 - E_inhibitor * 0.14 # Reaction rates
...
# --- Lattice Quest Sweep Configuration ---
# Building on previous success (multi-spot)
# Pushing D_E even higher to seek stripes/labyrinths
# Slightly adjusting alpha_base range
alpha_values = np.around(np.arange(0.23, 0.28 + 0.01, 0.01), decimals=3) # Range: 0.23 to 0.28, step 0.01 (6 values)
diffusion_E_values = np.around(np.array([12.0, 15.0, 18.0]), decimals=1) # New, higher D_E values (3 values)
...
ETA_BASE = 1.0
S_CONST = 1.0
BETA_BASE = 0.80 # Keeping beta_base constant
DIFFUSION_RATE_P = 0.5 # Keeping D_P constant
...
USE_STRUCTURED_SEED = True # Still using the multi-nuclei structured seed

# Prepare a list of all parameter combinations to run
param_combinations = []
for alpha in alpha_values:
    for d_e in diffusion_E_values:
        param_combinations.append((RESOLUTION, ITERATIONS, DT, alpha, BETA_BASE, ETA_BASE, S_CONST,
                                   DIFFUSION_RATE_P, d_e, INITIAL_NOISE_SCALE, ADD_NOISE_PERTURBATION,
                                   NOISE_PERTURBATION_INTERVAL, NOISE_PERTURBATION_SCALE, MICRO_FLUCTUATION_SCALE,
                                   USE_STRUCTURED_SEED))
...
def main():
    ...
    # This must be done at the very beginning of the main execution block
    multiprocessing.set_start_method('spawn', force=True)

    # Setup plotting grid
    num_alpha = len(alpha_values)
    num_d_e = len(diffusion_E_values)
    fig, axes = plt.subplots(num_alpha, num_d_e, figsize=(num_d_e * 5, num_alpha * 5)) # Adjusted figsize for clarity
    if num_alpha == 1 and num_d_e == 1:
        axes = np.array([[axes]])
    elif num_alpha == 1 or num_d_e == 1:
        axes = axes.reshape(num_alpha, num_d_e)

    print(f"Starting PEF Turing Pattern Lattice Quest Sweep (STANDALONE SCRIPT): {len(param_combinations)} simulations will be run.")
    ...
    if num_cores < 1:
        num_cores = 1
    print(f"Using {num_cores} CPU cores for parallel processing. Running lattice quest, Doctor!")
    ...
    for i in range(num_alpha):
        for j in range(num_d_e):
            pattern = results[result_idx]
            alpha = alpha_values[i]
            d_e = diffusion_E_values[j]

            ax = axes[i, j]
            ax.imshow(pattern, cmap='inferno', origin='lower')
            ax.set_title(f'α={alpha:.3f}, D_E={d_e:.1f}')
            ax.axis('off')
            result_idx += 1

    print("\nPEF Turing Pattern Lattice Quest Sweep (STANDALONE SCRIPT) complete. Analyze the grid of patterns!")
    plt.tight_layout()
    plt.show()

    print("\nLattice Quest Sweep finished. Observe how changes in alpha_base and diffusion_rate_E lead to different pattern types (spots, stripes, labyrinths, etc.). This is your expanded pattern taxonomy, Doctor!")
